...
# Installs: core + trellis-mathpack + trellis-undo + trellis-tui (all editable) + pytest, then runs the suite.
#
# Notes:
...
"$VENV_PY" -m pip install $PIP_FLAGS -e "$ROOT" -e "$ROOT/packages/trellis-mathpack" -e "$ROOT/packages/trellis-undo" -e "$ROOT/packages/trellis-tui" pytest pytest-asyncio
