...
- **Keyboard fill** (Part 8): `Ctrl+D` fills down, `Ctrl+R` fills right — within the selection from its first row/column, or with no selection from the cell above/left, Excel-exact. Formulas shift per lane (`$` pins hold), values copy, empty sources clear; the whole fill is one batch and one undo step, and your clipboard is never touched. No series extrapolation, on purpose: put `=A1+1` below a value and fill down — **the formula IS the series** (and recalc keeps it honest where a pasted 1,2,3 would go stale).
