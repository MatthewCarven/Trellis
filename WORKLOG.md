...
**Next pick-up:** **Part 5 #4 — read-only `SheetGrid`** (the heart — the Part 3 surface, consumed): window = `used_range()` ∪ min size (defaults decided there), grow-on-demand, A..Z/1..N headers, cell cursor, formula-bar mirroring, event-driven repaint incl. `sheet:batch`; Pilot tests. Sandbox note stands: textual 8.2.7 + pytest-asyncio installed system-level; TUI tests run from `packages/trellis-tui/` with `PYTHONPATH=../../src:src python3 -m pytest tests --basetemp=/tmp/... -p no:cacheprovider`.
