...
**Next pick-up:** **Part 5 #5 — editing**: the bar becomes an Input (replace/revise edits), commit keys (Enter/Tab/Shift variants), typed-input inference via the now-public `trellis.infer_value`, `Delete` clears, dirty flag from engine events; settle nav-Enter (lean revise-edit) and empty-commit (lean delete); prefill from `cell.formula`/`repr` per the #3 lossiness flag. Sandbox note stands (textual 8.2.7; run from `packages/trellis-tui/` with `PYTHONPATH=../../src:src`).
