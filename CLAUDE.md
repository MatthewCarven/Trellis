...
- **`packages/trellis-keymap/`** — the *keymap contract*, extracted from the TUI (S40): the textual-free key-language layer (`KeyPress`/`KeyContext`/`Action`/`Keymap`/`ExcelKeymap` + discovery under the `trellis_keymap.keymaps` entry point), zero dependencies like core. A frontend imports it to host key languages; a keymap package (`trellis-tui-vim`) imports only it to *be* one. The seam a second frontend (a planned GUI) shares with the TUI.
